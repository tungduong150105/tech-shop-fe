...
import { useAdminFilterOption, useUpdateAdminFilterOption, useAdminCategories } from '../../hooks'
...
export default function AdminFilterEdit() {
  ...
  useEffect(() => {
    if (filterOption) {
      setKey(filterOption.key || '')
      setLabel(filterOption.label || '')
      setValue(filterOption.value || '')
      setCategoryId(filterOption.category_id ? Number(filterOption.category_id) : null)
      setOrder(filterOption.order || 0)
      setIsActive(filterOption.is_active !== undefined ? filterOption.is_active : true)
    }
  }, [filterOption])
  ...
  if (!filterOption) {
    return <div className="p-6">Filter option not found</div>
  }
