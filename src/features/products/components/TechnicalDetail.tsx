...
const TechnicalDetail = ({ specs }: TechnicalDetailProps) => {
  ...
  return (
    <div className="w-full rounded-2xl bg-white p-5 sm:p-6 border-gray-100">
      <h2 className="text-xl sm:text-xl font-normal text-gray-900 mb-4">
        Technical Details
      </h2>
      ...
      <dl className="divide-y divide-gray-100">
        {specs?.slice(0, visibleCount).map((item, i) => (
          <div
            key={i}
            className={`flex justify-between py-3 px-3 rounded-md ${
              i % 2 === 0 ? 'bg-gray-100' : ''
            }`}
          >
            <dt className="text-gray-600 font-medium w-1/3">{item.label}</dt>
            <dd className="text-black font-light w-2/3 text-left">{item.value}</dd>
          </div>
        ))}
      </dl>
